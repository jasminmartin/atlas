...
class FileConsumerSpec extends FixtureAnyWordSpec with Matchers {
  ...
  "FileConsumerSpec" when {
    "Given a flat file directory structure" should {
      "Return a list of the files it contains" in { f =>
        LocalFileConsumer
          .isDirectory(f.flatDirectoryStructure)
          .get should contain theSameElementsAs f.flatDirectoryStructureFiles
      }
      "Filter out non-text files" in { f =>
        val allFiles: Option[List[File]] =
          LocalFileConsumer.isDirectory(f.flatDirectoryStructure)
        LocalFileConsumer.filterFileExtensions(
          allFiles.get,
          List(".txt")
        ) should contain theSameElementsAs f.flatDirectoryStructureTxtFiles
      }
      ...
      "Handle the case if a file does not have accessible permissions" in { f =>
        pending
      }
    }
    ...
    "Given a nested file directory structure" should {
      "Return a list of the files contained in all directories" in { f =>
        LocalFileConsumer
          .isDirectory(f.nestedDirectoryStructure)
          .get should contain theSameElementsAs f.nestedDirectoryStructureFiles
      }
      ...
      "Filter out non-text files" in { f =>
        ...
        LocalFileConsumer.filterFileExtensions(
          allFiles.get,
          List(".txt")
        ) should contain theSameElementsAs f.nestedDirectoryStructureTxtFiles
      }
      "Handle the case if no files are found" in { f =>
        val emptyDirectoryStructure: String =
          "src/test/Resources/TestData/household/inhabitants"
  ...
  override protected def withFixture(test: OneArgTest): Outcome = {
    ...
    val catPicture = new File(
      "src/test/Resources/TestData/household/catPicture.png"
    )
    ...
    val bathroom = new File(
      "src/test/Resources/TestData/household/rooms/bathroom.txt"
    )
    val horse = new File(
      "src/test/Resources/TestData/household/rooms/horse.jpg"
    )
    val dogPicture = new File(
      "src/test/Resources/TestData/household/rooms/dog.png"
    )
    ...
    val lion = new File(
      "src/test/Resources/TestData/household/wildlife/lion.txt"
    )
    val table = new File("src/test/Resources/TestData/household/table.txt")
    val flatDirectoryStructure: String =
      "src/test/Resources/TestData/household/rooms"
    ...
    try {
      this.withFixture(
        test.toNoArgTest(
          FixtureParam(
            flatDirectoryStructureFiles: List[File],
