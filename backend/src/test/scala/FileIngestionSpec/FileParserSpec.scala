...
class FileParserSpec extends AnyWordSpec with Matchers with BeforeAndAfterEach {
  ...
  "FileConsumerSpec" when {
    "Given a nested directory structure" should {
      "Return a list of files" in {
        LocalFileParser.isDirectory(utils.testingDirectory).getOrElse(fail("Cannot get top level")) should contain theSameElementsAs utils.allFiles
      }
